...
Guia abrangente de otimização de desempenho para aplicativos React e Next.js, mantido pela Vercel. Contém 57 regras em 8 categorias, priorizadas por impacto para orientar a refatoração automatizada e a geração de código.
...
- `server-auth-actions` - Autentica ações do servidor como rotas de API
...
- `server-dedup-props` - Evite serialização duplicada em adereços RSC
...
- `client-localstorage-schema` - Versão e minimiza os dados do localStorage
...
- `rerender-memo-with-default-value` - Hoist adereços não primitivos padrão
...
- `rerender-derived-state-no-effect` - Deriva o estado durante a renderização, não os efeitos
...
- `rerender-simple-expression-in-memo` - Evite memorando para primitivas simples
- `rerender-move-effect-to-event` - Coloque lógica de interação em manipuladores de eventos
- `rerender-transitions` - Use startTransition para atualizações não urgentes
- `rerender-use-ref-transient-values` - Use refs para valores frequentes transitórios
...
- `rendering-hydration-suppress-warning` - Suprime incompatibilidades esperadas
...
- `rendering-usetransition-loading` - Prefira useTransition para carregar o estado
...
- `advanced-init-once` - Inicializa o aplicativo uma vez por carregamento do aplicativo
